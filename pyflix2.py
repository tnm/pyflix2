...
## This library here is under the GPL, because that's what the original
## pyflix library is under.

## I'll probably end up writing an entirely new client at some point. -- Ted
...
class NetflixUserQueue(object):
    ...
    def get_available_disc(self, sort=None, start_index=None, max_results=None, updated_min=None, queue_type='disc'):
    ...
    def get_available_instant(self, sort=None, start_index=None, max_results=None, updated_min=None, queue_type='instant'):
    ...
    def add_title_instant(self, disc_info=[], urls=[], queue_type='instant', position=None):
        ...
        request_url = '/users/%s/queues/instant' % (access_token.key)
        if not urls:
            for disc in disc_info:
                urls.append( disc['id'] )
        parameters['title_ref'] = ','.join(urls)
